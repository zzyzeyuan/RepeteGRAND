...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        """-----------test code---------------------"""
        #     for i in range(140):
        #         second = labels[train_idx][i]
        #         first = logits[k][train_idx][i][second]
        #         first = -first
        #         losssum += first
        #     losssum = (losssum/140)
        #     manulloss += losssum
        # print("手动计算的loss为： %f" % (manulloss/args.sample))
        """-----------test code---------------------"""
        ...
        print("supervised loss是%f, consistency loss是%f" % (loss_sup, loss_consis))
        loss_train = loss_sup + loss_consis
        # 为什么直接取logits[0]? 不是sample了4次吗
        acc_train = th.sum(logits[0][train_idx].argmax(dim=1) == labels[train_idx]).item() / len(train_idx)

        """---------test code------- should be removed"""
        # acc_train0 = th.sum(logits[0][train_idx].argmax(dim=1) == labels[train_idx]).item() / len(train_idx)
        # acc_train1 = th.sum(logits[1][train_idx].argmax(dim=1) == labels[train_idx]).item() / len(train_idx)
        # acc_train2 = th.sum(logits[2][train_idx].argmax(dim=1) == labels[train_idx]).item() / len(train_idx)
        # acc_train3 = th.sum(logits[3][train_idx].argmax(dim=1) == labels[train_idx]).item() / len(train_idx)
        #
        # print("四次sample的acc_train分别为： %f, %f, %f, %f." % (acc_train0, acc_train1,acc_train2,acc_train3))
        """----------test code------- should be removed"""
        # backward
        opt.zero_grad()
        loss_train.backward()
